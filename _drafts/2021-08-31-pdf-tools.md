...
When extracting URLs, it's important to make a distinction between the following two cases:

1. URLs that are encoded as a "link annotation", which is a data structure in PDF that results in a clickable link
2. Non-clickable URLs that are just part of the body text.
...
I did some limited testing with this tool in 2016. One issue I ran into is that pdfx [truncates URLS that span more than one line](https://github.com/metachris/pdfx/issues/21). As of 2021, this issue hasn't been fixed so far, which seriously limits the usefulness of this (otherwise very interesting) tool.

### Other URL extraction tools
...
## Show information about embedded images

### List images with  with pdfimages

The aforementioned *pdfimages* tool is also useful for getting an overview of all embedded images in a PDF, and their main characteristics (width, height, colour, encoding, resolution and size). just user the `-list` option as shown below:  
