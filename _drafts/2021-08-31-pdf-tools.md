...
Nevertheless, this tool is still useful for detecting various issues, especially in conjunction with the `--verbose` option. Here's an example command-line:
...
Ghostscript will report quite detailed information in case of any problems with the input file. As an example, here's the output for a PDF with a truncated document trailer:
...
## Validation against PDF/A profile
