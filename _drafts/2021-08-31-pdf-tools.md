...
Invoking QPDF (see above) with its `--check` option performs checks on the overall file structure. QPDF does not provide full-fledged validation, and the [documentation](http://qpdf.sourceforge.net/files/qpdf-manual.html) states that:
...
Nevertheless, the tool is still very useful for detecting various issues, especially in conjunction with the `--verbose` option. Here's an example command-line:
...
Perhaps leave out bc:

1. Reliance on exit code (not obvious to newbies)
2. Quick test showed truncated PDFs still gave exit code 0!

### Pdfinfo

The *pdfinfo* tool that is part of Xpdf and Poppler 

The [Poppler](https://poppler.freedesktop.org/)
...
<https://docs.verapdf.org/cli/feature-extraction/>
...
## Validation against profile (PDF/A)
