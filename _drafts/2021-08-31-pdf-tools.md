...
Before diving into any specific tasks, I'll start this post by listing a number of general-purpose PDF tools and toolkits. Each of these are capable of various tasks (including some that I won't explicitly address in this post), and they can be seen as "Swiss army-knives" of PDF processing. Whenever I need to get some PDF processing or analysis done and I'm not sure what tool to use, these are the tools are usually my starting point. In the majority of cases at least one of them turns out to have the functionality I'm looking for, so it's a good idea to check them out iof you're not familiar with them already:
...
- [Pdfcpu](https://github.com/pdfcpu/pdfcpu) is a PDF processing library (which includes a CLI tool) that is written in the *Go* language. It supports all PDF versions up to PDF 1.7 (ISO-32000).
